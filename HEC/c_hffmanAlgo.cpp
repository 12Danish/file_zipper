...
void huffman::createTree()
{
    ...
    Node *left, *right;
    CustomMinHeap<Node *> tempPQ(minHeap);
    while (tempPQ.size() != 1)
    {
        left = tempPQ.top();
        tempPQ.pop();
...
void huffman::saveEncodedFile()
{
    ...
    1 - Character representation of the size of the huffman tree is being stored within in which is  astring
    2 - looping over all the characters and doing the following:
    - adding their data(character) to the string 'in'
    - adding their corresponding codes but with 127 total bits(zeros are being prefixed)
...
void huffman::saveDecodedFile()
{
    ...
    CustomVector<unsigned char> text;
